...
export async function searchWeb(query: string, history: Array<{ role: "user" | "assistant"; content: string }>): Promise<{ answer: string; candidates: Array<Omit<Candidate, "id">> }> {
  ...
  const baseInput: ResponsesInputMessage[] = [{ role: "system", content: [{ type: "input_text", text: systemText }] }, ...historyInput, { role: "user", content: [{ type: "input_text", text: query }] }];
  const request = async (retry: boolean) => {
    const input: ResponsesInputMessage[] = retry ? [...baseInput, { role: "user", content: [{ type: "input_text", text: "The public web search is complete. Return the final structured JSON object now. Do not return tool calls, reasoning, or prose outside the JSON object." }] }] : baseInput;
