...
export function deterministicDecision(message: string): ReplyDecision | null {
  ...
  if (/support instagram|work with instagram|instagram.*support/i.test(text)) return { mode: "AUTO_SAFE", intent: "capability_faq", confidence: 0.96, reason: "Instagram support is a low-risk capability question; verified facts are required and automatic sending is still disabled.", missingInformation: [], riskFlags: [], autoSafeEligible: true };
