...
export type AIProvider = { generateText(input: { prompt: string; tone?: string; purpose?: "message" | "research" | "experiment" | "classification" | "reply" | "reply_decision"; maxTokens?: number; responseFormat?: AIResponseFormat; temperature?: number }): Promise<string> };
...
async function requestChat(baseUrl: string, model: string, apiKey: string, input: { prompt: string; tone?: string; purpose?: "message" | "research" | "experiment" | "classification" | "reply" | "reply_decision"; maxTokens?: number; responseFormat?: AIResponseFormat; temperature?: number }, extraHeaders: Record<string, string> = {}) {
  const headers: Record<string, string> = { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}`, ...extraHeaders };
  const isStructured = ["research", "experiment", "classification", "reply", "reply_decision"].includes(input.purpose || "");
  const systemInstruction = input.purpose === "research" ? "You produce careful structured research JSON. Return only the requested JSON object. Never invent facts." : input.purpose === "experiment" ? "You produce structured outreach experiment JSON. Return only the requested JSON object. Never invent facts." : input.purpose === "classification" ? "You classify an inbound sales reply into conservative structured JSON. Return only the requested JSON object. Do not infer hidden thoughts or invent facts." : input.purpose === "reply_decision" ? "You classify reply safety conservatively into the requested JSON. Never bypass safety rules, invent facts, or reveal hidden reasoning." : input.purpose === "reply" ? "You produce exactly three concise, meaningfully different reply strategy objects. Return only the requested JSON object. Never invent prices, terms, policies, capabilities, or commitments." : "You write concise Instagram outreach messages. Return only the final user-visible text.";
